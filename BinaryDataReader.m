...
//  http://davepeck.org/
//
//  This class makes it a little easier to read sequential binary files in Objective-C.
//
...
@interface BinaryDataReader (Private)
...
@implementation BinaryDataReader (Private)
...
-(NSException *)buildScanException
{
    return [NSException exceptionWithName:@"BinaryDataReaderException" reason:@"Failure reading desired information from the bytes." userInfo:nil]; 
}

@end


@implementation BinaryDataReader

+(id)binaryDataScannerWithData:(NSData*)data littleEndian:(BOOL)littleEndian defaultEncoding:(NSStringEncoding)defaultEncoding
{
    return [[[BinaryDataReader alloc] initWithData:data littleEndian:littleEndian defaultEncoding:defaultEncoding] autorelease];
}
